...
describe('findWinningDecompositions', () => {
  it('returns decompositions across both patterns without a verdict wrapper', () => {
    const decompositions = findWinningDecompositions({
      concealed: tiles('1m 1m 2m 2m 3m 3m 4m 4m 5m 5m 6m 6m 7m 7m east east east'),
      exposedMelds: [],
    });
    expect(decompositions.some((one) => one.pattern === 'STANDARD')).toBe(true);
    expect(decompositions.some((one) => one.pattern === 'SEVEN_PAIRS')).toBe(true);
  });
});
