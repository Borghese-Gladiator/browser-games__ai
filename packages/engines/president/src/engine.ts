...
export type PresidentAction = { cards?: string[]; pass?: boolean };
...
export function createGame(options: Record<string, unknown> = {}): PresidentState {
  return {
...
function clone(state: PresidentState): PresidentState {
  return {
    ...state,
...
export function addPlayer(state: PresidentState, { id, name }: SeatInit): PresidentState {
...
function nextInPlaySeat(state: PresidentState, fromSeat: number): number {
...
function inPlayPlayers(state: PresidentState): PresidentPlayer[] {
  return state.players.filter((p) => !p.finished);
}

export function startRound(state: PresidentState): PresidentState {
...
function validGroup(hand: string[], cards: string[] | undefined): cards is string[] {
...
export function legalPlays(state: PresidentState, seat: number): string[][] {
  if (
    state.phase !== 'playing' ||
...
function canPass(state: PresidentState): boolean {
...
function resetTrick(next: PresidentState): void {
...
function assignRoles(next: PresidentState): void {
...
export function publicState(state: PresidentState, forSeat: number): Record<string, unknown> {
...
export function getOutcome(state: PresidentState) {
