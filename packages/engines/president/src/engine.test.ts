...
function withHands(hands: string[][]) {
...
const idAt = (state: PresidentState) => state.players[state.activeSeat].id;
...
describe('finishing order and role assignment', () => {
  it('records finish order and assigns President / VP / Scum', () => {
    ...
    while (state.phase === 'playing') {
      ...
      const plays = publicState(state, state.activeSeat).legalPlays as string[][];
